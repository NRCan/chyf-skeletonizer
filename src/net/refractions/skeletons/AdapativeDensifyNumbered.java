...
// this a smarter version of AdapativeDensified -- assumes all the coordinates in the Polygon are Numbered
//  SkelCoordinates.  See skeletonizer.java for more detail on numbering.
//
//  Basically, this will NOT densify if the "near" edge has the same number. 
public class AdapativeDensifyNumbered
{
	...
	public AdapativeDensifyNumbered(Polygon p)
	{
		inputPolygon = p;
		 ...
		 */
		public AdapativeDensifyNumbered(Polygon p,double factor)
		{
			 inputPolygon = p;
	...
	//
	// numbered version.  If e has both coords with same #, then we might be able to save 
	//     points by removing other edges that are numbered the same.
public void removeRedundant(AdaptiveDensifyEdge e, List nearbyEdges)
{
	...
	if (Ef != null)
	{
		...
		if (angle_for >135)   // for very curvy lines, we densify alot!
			toRemove.add( Ef);
		else
			Ef = null; // see comment below
	}
	...
	if (Er != null)
	{
		...
		if (angle_bac >135)
			...
			Er = null; // dont trace backwards - this could cause problems.  The Numbering step will remove "bad" edges
	}
	...
	//backwards
	 hot = Er; // current edge we're looking at
		...
		// handle numbering extension
		SkelCoordinate sc1,sc2;
		sc1 = (SkelCoordinate) e.c1;
		sc2 = (SkelCoordinate) e.c2;
		

		
		boolean canDoSomeRemoves = (!(sc1.entryPoint)) && (!(sc2.entryPoint)) && (sc1.metaNum == sc2.metaNum);
		if (canDoSomeRemoves)
		{
			Iterator it_nearby = nearbyEdges.iterator();
			while (it_nearby.hasNext())
			{
						AdaptiveDensifyEdge  e_other = (AdaptiveDensifyEdge) it_nearby.next();
						SkelCoordinate sc1_other,sc2_other;
						sc1_other = (SkelCoordinate) e_other.c1;
						sc2_other = (SkelCoordinate) e_other.c2;
						
					boolean remove =  (!(sc1_other.entryPoint)) && (!(sc2_other.entryPoint)) && (sc1_other.metaNum == sc2_other.metaNum);
					remove = remove && (sc1_other.metaNum == sc1.metaNum);
					if (remove)
					{
						it_nearby.remove();
					}				
